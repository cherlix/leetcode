...
public:
    int findRadius(vector<int>& houses, vector<int>& heaters) {
        ...
        for (auto house : houses) {
            if (house <= heaters[0]) {
                max_distance = max(max_distance, heaters[0] - house);
            } else if (house >= heaters[heaters.size() - 1]) {
                max_distance = max(max_distance, house - heaters[heaters.size() - 1]);
            } else {
                auto it = lower_bound(heaters.begin(), heaters.end(), house);
                auto prev_it = it - 1;
                max_distance = max(max_distance, max(*it - house, house - *prev_it));
            }
        }
        return max_distance;
    }
